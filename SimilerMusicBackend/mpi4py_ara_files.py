...
def file(song,out):
    ...
    gc.enable()
    filelist = []
    # for filename in Path('/root/sim/music').glob('**/*.mp3'):
    #     filelist.append(filename)
    # for filename in Path('/root/sim/music').glob('**/*.wav'):
    #     filelist.append(filename)  
    # for filename in Path('/root/sim/music').glob('**/*.aiff'):
    #     filelist.append(filename) 
    # for filename in Path('/root/sim/music').glob('**/*.flac'):
    #     filelist.append(filename) 
    # for filename in Path('/root/sim/music').glob('**/*.ogg'):
    #     filelist.append(filename) 
    ...
    def parallel_python_process(process_id, cpu_filelist):
        ...
        for file_name in cpu_filelist:
            ...
            with open(out + ".files", "a") as myfile:
        ...
        for index in range(startjob + rank, int(maxparts), size):
            if index < parts:        
                ...
                with open(out + ".files", "a") as myfile:
                    ...
                    myfile.close()
                print(filelist[0])
                parallel_python_process(index, filelist[int(starti):int(endi)])
                gc.collect()
        gc.enable()
        gc.collect()
