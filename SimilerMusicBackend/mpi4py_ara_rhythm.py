...
import gc # garbage collector
 # unicode csv library (installed via pip install unicsv)
...
def extract_all_files_generic(in_path,
    ...
    if maxparts > parts:
        maxparts = parts
    for index in range(startjob + rank, int(maxparts), size):
        # if index < parts:        
        starti = start+index*step
        endi = min(start+(index+1)*step, end)
        print("calling process  " + str(rank) + " index " + str(index) + " size " + str(size) + " starti " + str(starti) + " endi " + str(endi))
        # 書き換え
        # extract_all_files(filelist[int(starti):int(endi)], in_path, out_file + str(index), feature_types, label, append, no_extension_check, force_resampling, out_HDF5, log_AudioTypes, log_Errors, verbose)
        extract_all_files(filelist[int(starti):int(endi)], in_path, out_file , feature_types, label, True, no_extension_check, force_resampling, out_HDF5, log_AudioTypes, log_Errors, verbose) 
        gc.collect()
    gc.enable()
    gc.collect()
    return 
...
def extract_all_files(filelist,
    ...
    if out_file: # only if out_file is specified
        ...
        if log_AudioTypes:
            ...
            audio_logwriter = csv.writer(audio_logfile) #, quoting=csv.QUOTE_ALL)

        if log_Errors:
            err_log_filename = out_file + '.errors.log'
            error_logfile = open(err_log_filename, 'w') # TODO allow append mode 'a'
            error_logwriter = csv.writer(error_logfile) #, quoting=csv.QUOTE_ALL)
    ...
    for fil in filelist:  # iterate over all files
        try:
            ...
            if out_file:
                ...
                # FeatureWriter.write_features("/root/sim/music/" + id,feat,id2)
