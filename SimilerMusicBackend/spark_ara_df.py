...
#実行メソッド追加
def search_similer_music(sc,sqlContext,song2,kv_rp,kv_rh,kv_bh,notes,mfccDfMerged,chromaDf,list_to_vector_udf,negjs,nanjs,nonpdjs,negskl,nanskl,noninskl):
    ...
    def get_euclidean_mfcc(vec1, vec2):
        ...
        mean1 = vec1[0:13]
        cov1 = np.empty([13,13])
        print("this is lain in the wired")
        print(len(vec1))
        # print(vec1.col("id"))
    ...
    def get_neighbors_mfcc_euclidean(song):
        df_vec = mfccDfMerged
        filterDF = df_vec.filter(df_vec.id == song)
        print("filterDF print")
        # print(filterDF.show(truncate=False))
        comparator_value = Vectors.dense(filterDF.collect()[0][1]) 
        ...
        # result = df_vec.withColumn('distances_mfcc', distance_udf(F)).select("id", "distances_mfcc")
    ...
    def get_nearest_neighbors_full(song, outname):
        ...
        mergedSim = mergedSim.withColumn('aggregated', ( F.when(mergedSim.scaled_rp > 1,0.18).otherwise(mergedSim.scaled_rp) + mergedSim.scaled_skl + mergedSim.scaled_js + mergedSim.scaled_rh) / 4)
        ...
        mergedSim.show(3000,truncate=False)
    ...
    # song1 = "/root/sim/music/AC_DC/Back In Black/02 Shoot To Thrill.mp3"
    # song2 = "/root/sim/music/The Beatles/1967-1970 [Disc 2]/12 Let It Be.mp3"
    # song2 = "/root/sim/music/Aerosmith/O, Yeah! Ultimate Aerosmith Hits [Disc 2]/09 I Don't Want To Miss A Thing.mp3"
    ...
    song2 = song2.replace(";","").replace(".","").replace(",","")
    # tic1 = int(round(time.time() * 1000))
    ...
    # res2 = get_nearest_neighbors_fast(song2, "DF_FULL_SONG2.csv")
