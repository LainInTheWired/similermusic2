...
gc.enable()
filelist = []
for filename in Path('/root/sim/music/out').glob('**/*.mp3'):
...
def parallel_python_process(process_id, cpu_filelist, f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh):
    ...
    if f_mfcc_euclid == 1:
        with open("featuresout/out" + str(process_id) + ".mfcc", "w") as myfile:
            myfile.write("")
            myfile.close()
    if f_mfcc_kl == 1:
        with open("featuresout/out" + str(process_id) + ".mfcckl", "w") as myfile:
            myfile.write("")
            myfile.close()
    if f_chroma == 1:
        with open("featuresout/out" + str(process_id) + ".chroma", "w") as myfile:
            myfile.write("")
            myfile.close()
    if f_bh == 1:
        with open("featuresout/out" + str(process_id) + ".bh", "w") as myfile:
            myfile.write("")
            myfile.close()
    if f_notes == 1:
        with open("featuresout/out" + str(process_id) + ".notes", "w") as myfile:
            myfile.write("")
            myfile.close()
    count = 1
